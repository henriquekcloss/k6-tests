...
- As much as we write k6 tests in javascript, in background it runs with Go. It means that k6 will not accept javascript librarys directly. In order to do so, I've configured Babel to transpile the code to javascript, this way we are able to use our dataFaker with chance or any other javascript library that you want. To do so, you need to pay attention to the file "webpack.config.js".
  ...
  - webpack.config.js:
    ...
    - The "example" is the name that you are given to your transpiled file saved in the dist folder, it will automatically add .test.js at the end of the name, and the path "./tests/api/scaleUpAndDown.test.js" is the location of your test file.
